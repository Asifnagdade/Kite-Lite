...
# --- 2. MASTER DATA (As per your MCX screenshot) ---
MASTER_DATA = {
    "NSE": {
...
# --- 3. STYLING (Fixed unsafe_allow_html) ---
st.markdown("""
    <style>
...
# --- 4. SESSION STATE (Admin Restored) ---
...
# --- 5. PRICE ENGINE (Simulated High Bid/Ask) ---
def get_market_data(ticker, is_mcx=False):
    try:
        ...
        if is_mcx:
            ltp = ltp * 83.80 # INR Scaling
            data['Close'] *= 83.80
        
        # Real-time Flicker
        ltp += random.uniform(-0.15, 0.15)
        bid = round(ltp - 0.75, 2)
        ask = round(ltp + 0.75, 2) # Best Ask for Market Buy
...
with st.sidebar:
    ...
    to_add = st.selectbox("Add Script", list(MASTER_DATA[seg].keys()))
...
with t1:
    ...
    active = st.radio("Select View", [sel_nse, sel_mcx], horizontal=True)
    base = active.split(' ')[0]
    is_mcx = base in MASTER_DATA["MCX"]
    yf_ticker = MASTER_DATA["MCX"].get(base) or MASTER_DATA["NSE"].get(base)

    df, ltp, bid, ask = get_market_data(yf_ticker, is_mcx)
    ...
    if ltp > 0:
        ...
        with col_main:
            ...
            d1, d2 = st.columns(2)
            d1.metric("LTP", f"₹{ltp}")
            d2.metric("Market Buy Price (Ask)", f"₹{ask}")

        with col_order:
            st.write("### Order Window")
            o_type = st.radio("Order", ["Market", "Limit"])
            prod = st.radio("Product", ["Intraday (500x)", "Delivery (60x)"])
            qty = st.number_input("Qty", min_value=1, step=1)
            
            # Logic: Market Buy executes at ASK (High Bid)
            ...
            if st.button("BUY", type="primary"):
                if st.session_state.user_db[u_id]["balance"] < margin: st.error("Low Funds!")
                    ...
                    st.session_state.portfolio.append({
                        "Symbol": active, "Price": exec_p, "Qty": qty, 
...
with t3:
    st.info("Admin Account: asifnagdade | Leverage: 500x Intraday, 60x Delivery | Market Orders execute at Best Ask.")

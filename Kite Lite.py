...
# --- 2. DATA STORAGE (Isse IDs save rahengi refresh par bhi) ---
...
if not st.session_state.logged_in_user:
    ...
    if st.button("Login"):
        # Yahan check ho raha hai current database se
        if u_in in st.session_state.user_db:
            ...
            st.error(f"❌ ID '{u_in}' nahi mili! Admin se banwayein.")
    st.stop()
...
if u_role == "admin":
    ...
    with tabs[0]: # User ID Control
        st.subheader("Create/Delete Client ID")
    ...
    with tabs[3]: # Delete History
        st.subheader("Permanent Trade History Deletion")
...
# --- 5. CLIENT SECTION ---
    ...
    if st.session_state.user_db[u_id].get("first_login", True):
        ...
        if st.button("Update"):
            ...
            st.success("Updated! Now login again.")
